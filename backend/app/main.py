...
from .services.watchdog import WatchdogEngine
...
watchdog = WatchdogEngine(store, queue, controls)
reflection = ReflectionEngine(commerce, memory)
sales_service = SalesService(db)
analytics_service = AnalyticsService(db)
orchestrator = AutonomousOrchestrator(commerce, store, queue, controls, memory=memory)

async def run_watchdog() -> dict[str, int]:
    result = await watchdog.run_once()
    await store.record_watchdog_check("jobs", result, status="ok")
    if result.get("incidents"):
        await commerce.record_event(
            "watchdog_incidents",
            {"incidents": result["incidents"], "recovered": result.get("recovered", 0)},
            idempotency_key=f"watchdog:{datetime.now().astimezone().strftime('%Y%m%d%H%M')}",
        )
    return result

async def run_autonomous_orchestration() -> dict:
    await discover_public_signals(commerce, limit=8)
    return await orchestrator.run_cycle(limit=3)

async def autonomous_cycle_once() -> dict[str, object]:
    watchdog_result = await run_watchdog()
    orchestration_result = await run_autonomous_orchestration()
    return {"watchdog": watchdog_result, "orchestration": orchestration_result}
    ...
    while True:
        try:
            ...
            if not status.get("kill_switches", {}).get("global") and not status.get("settings", {}).get("pause_radar"):
                ...
                if reflection_counter >= 4:
                    await reflection.run_once(trigger_type="scheduled")
                    reflection_counter = 0
        except asyncio.CancelledError:
            raise
        except Exception as exc:
            try:
                fingerprint = f"autonomous-loop:{type(exc).__name__}:{str(exc)[:120]}"
                await store.upsert_incident(fingerprint, {
                    "kind": "autonomous_loop_failure",
                    "severity": "critical",
                    "status": "open",
                    "summary": "Autonomous heartbeat failed",
...
async def system_status() -> dict[str, object]:
    ...
    return {"api": True, "ai": settings.ai_configured, "ai_provider": settings.effective_ai_provider, "supabase": supabase_ok, "queue": "redis" if queue.uses_redis else ("supabase" if settings.supabase_url and settings.supabase_secret_key else "unconfigured"), "hotmart_credentials": bool(settings.hotmart_client_id and settings.hotmart_client_secret), "telegram_credentials": bool(settings.telegram_bot_token and settings.telegram_chat_id)}
...
async def chat(request: ChatRequest) -> ChatResponse:
    ...
    try:
        result = await ai_gateway.complete(request.message, system=("You are Hermes Pro, an autonomous commerce operations assistant. Be concise, truthful, and never invent sales, integrations, publication, or completed jobs. " f"The current server date and time is {temporal_context}. Use server time when asked for current time."))
    except Exception as exc:
...
async def jobs() -> list[JobResponse]:
    try: rows = await store.list_jobs()
    except SupabaseError: return []
    return [job_response(row) for row in rows]

@app.get("/api/v1/jobs/{job_id}", response_model=JobResponse)
async def get_job(job_id: str) -> JobResponse:
    try: row = await store.get_job(job_id)
    except SupabaseError as exc: raise HTTPException(status_code=503, detail="Persistência Supabase não configurada.") from exc
...
async def products() -> list[ProductResponse]:
    try: rows = await store.list_products()
    except SupabaseError: return []
...
async def get_product(product_id: str) -> ProductResponse:
    try: row = await store.get_product(product_id)
    except SupabaseError as exc: raise HTTPException(status_code=503, detail="Persistência Supabase não configurada.") from exc
...
async def create_opportunity(request: OpportunityCreateRequest) -> OpportunityResponse:
    try: row = await commerce.create_opportunity(request.model_dump(exclude_none=True), request.idempotency_key)
    except SupabaseError as exc: raise HTTPException(status_code=503, detail="Persistência de oportunidades indisponível.") from exc
...
async def opportunities() -> list[OpportunityResponse]:
    try: rows = await commerce.list_opportunities()
    except SupabaseError as exc: raise HTTPException(status_code=503, detail="Persistência de oportunidades indisponível.") from exc
    return [OpportunityResponse(**row) for row in rows]

@app.get("/api/v1/events")
async def events() -> list[dict]:
    try: return await commerce.list_events()
    except SupabaseError as exc: raise HTTPException(status_code=503, detail="Persistência de eventos indisponível.") from exc

@app.get("/api/v1/expenses")
async def expenses() -> list[dict]:
    try: return await commerce.list_expenses()
    except SupabaseError as exc: raise HTTPException(status_code=503, detail="Persistência de custos indisponível.") from exc

@app.get("/api/v1/incidents")
async def incidents() -> list[dict]:
    try: return await store.list_incidents()
    except SupabaseError as exc: raise HTTPException(status_code=503, detail="Persistência de incidentes indisponível.") from exc

@app.get("/api/v1/memory")
async def memories() -> list[dict]:
    try: return await memory.retrieve(limit=100)
    except SupabaseError as exc: raise HTTPException(status_code=503, detail="Memória persistente indisponível.") from exc

@app.get("/api/v1/watchdog")
async def watchdog_status() -> dict[str, object]:
    try:
        rows = await db.select("hermes_watchdog_checks", params={"select": "*", "order": "updated_at.desc", "limit": "20"})
        return {"checks": rows, "stale_after_seconds": watchdog.stale_after_seconds}
    except SupabaseError as exc: raise HTTPException(status_code=503, detail="Watchdog indisponível.") from exc

@app.get("/api/v1/reflection")
async def reflection_status() -> dict[str, object]:
    try:
        rows = await db.select("hermes_reflections", params={"select": "*", "order": "created_at.desc", "limit": "20"})
        return {"runs": rows}
    except SupabaseError as exc: raise HTTPException(status_code=503, detail="Reflexão indisponível.") from exc

@app.get("/api/v1/expenses")
async def expenses_duplicate_guard() -> list[dict]:
    try: return await commerce.list_expenses()
    except SupabaseError as exc: raise HTTPException(status_code=503, detail="Persistência de custos indisponível.") from exc
...
async def sales(range_start: date | None = None, range_end: date | None = None) -> SalesSummary:
    try: return SalesSummary(**await sales_service.summary(range_start, range_end))
    except SupabaseError as exc: raise HTTPException(status_code=503, detail="Persistência de vendas indisponível.") from exc
...
async def analytics(range_start: date | None = None, range_end: date | None = None) -> AnalyticsResponse:
    try: return AnalyticsResponse(**await analytics_service.insights(range_start, range_end))
    except SupabaseError as exc: raise HTTPException(status_code=503, detail="Persistência de analytics indisponível.") from exc
...
async def dashboard() -> DashboardResponse:
    ...
    return DashboardResponse(products=len(products_list), active_jobs=sum(row["status"] in {"pending", "running", "retrying"} for row in jobs_list), completed_products=sum(row["status"] in {"completed", "content_ready", "ready_to_sell"} for row in products_list), revenue=sales_data.get("revenue"), sales=sales_data.get("orders"), integrations=integration_service.status())